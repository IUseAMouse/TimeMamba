...
class MambaForecastingModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append(
            {"loss": loss, "mae": mae, "y": y, "y_hat": y_hat}
        )
    ...
    def __init__(
        ...
        if task_type in ["classification", "multi_task"]:
            ...
            self.classification_head = nn.Sequential(
                nn.AdaptiveAvgPool1d(1), nn.Flatten(), nn.Linear(d_model, num_classes)
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.task_type == "forecasting":
            ...
            if forecast_target is not None:
                ...
                loss += forecast_loss
                self.log(
                    "train_forecast_loss", forecast_loss, prog_bar=True, sync_dist=True
                )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.task_type == "forecasting":
            ...
            if forecast_target is not None:
                ...
                self.log(
                    "val_forecast_loss", forecast_loss, prog_bar=True, sync_dist=True
                )
                self.log(
                    "val_forecast_mae", forecast_mae, prog_bar=True, sync_dist=True
                )
